...
{
    fprintf(stderr,
            "Usage: %s <graph_file> <walk_length_K> <damping_ratio_D> <threads> [directed|undirected]\n",
            prog);
    fprintf(stderr,
...
#pragma omp parallel
    {
        ...
        RngState rng;
        uint64_t node;
        uint64_t seed = (uint64_t)time(NULL) ^ (0x9e3779b97f4a7c15ULL * (uint64_t)(tid + 1));

        rng_seed(&rng, seed);

        for (node = (uint64_t)tid; node < node_count; node += (uint64_t)threads)
        {
...
#pragma omp critical
    ...
    for (i = 5; i < node_count; i++)
    {
        ...
        candidate.visits = visit_counts[i];
